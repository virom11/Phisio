...
def asymmetry(predictor_model, file_name):
    pose_landmarks = scriptsVector.face_aligner_func(
        predictor_model, file_name)
    i = 0
    asymmetry = 0
    while i < 68:
        if((i > 16) and (i != 27) and (i != 33)):
            ...
            asymmetry += (pose_landmarks.part(i).y-pose_landmarks.part(27).y)*(pose_landmarks.part(33).x-pose_landmarks.part(
                27).x)-(pose_landmarks.part(33).y-pose_landmarks.part(27).y)*(pose_landmarks.part(i).x-pose_landmarks.part(27).x)
        i += 1
    if(asymmetry > 0):
        left = 100
        right = 0
...
def nose_size(predictor_model, file_name, pose_landmarks):
    ...
    if (pose_landmarks.part(33).x > pose_landmarks.part(34).x) or (pose_landmarks.part(33).x < pose_landmarks.part(32).x):
            ...
            while(y <= max_y):
                ...
                if cords in data[0]:
                    ...
                    if ([cords[0], cords[1]-1] in data[0]):
                        delta = abs(data[1][data[0].index(
                            [cords[0], cords[1]-1])]-data[1][data[0].index(cords)])
                        counter += 1
                        sum_delta += delta
                    ...
                    if [cords[0]-1, cords[1]] in data[0]:
                        delta = abs(data[1][data[0].index(
                            [cords[0]-1, cords[1]])]-data[1][data[0].index(cords)])
                        counter += 1
                        sum_delta += delta
                    ...
                    if ([cords[0]+1, cords[1]+1] in data[0]):
                        delta = abs(data[1][data[0].index(
                            [cords[0]+1, cords[1]+1])]-data[1][data[0].index(cords)])
                        counter += 1
                        sum_delta += delta
        ...
        cl = [(0, 0, 0), (50, 50, 50), (100, 100, 100), (150, 150, 150), (0, 0, 63), (0, 0, 126), (0, 0, 189), (0, 0, 255), (0, 63, 0), (0, 126, 0),
               (0, 189, 0), (0, 255, 0), (63, 63, 0), (126, 126, 0), (189, 189, 0), (255, 255, 0), (63, 0, 0), (126, 0, 0), (189, 0, 0), (255, 0, 0)]
        ...
        graph1 = plt.plot(x_list[0:3], color_list[0:3],
                          color='blue', label='< 3')
        graph1 = plt.plot(x_list[2:7], color_list[2:7],
                          color='green', label='> 3 & < 7')
        graph1 = plt.plot(x_list[6:12], color_list[6:12],
                          color='yellow', label='< 7 & < 12')
        graph1 = plt.plot(
            x_list[11:20], color_list[11:20], color='red', label='> 12')
...
def nose_wings(predictor_model, file_name, pose_landmarks):
    ...
    a = 27
    b = 30
    distance_1 = scriptsVector.distance(pose_landmarks.part(27).x, pose_landmarks.part(
        27).y, pose_landmarks.part(0).x, pose_landmarks.part(0).y)
    distance_2 = scriptsVector.distance(pose_landmarks.part(27).x, pose_landmarks.part(
        27).y, pose_landmarks.part(16).x, pose_landmarks.part(16).y)
    ...
    limit_y1 = round(pose_landmarks.part(
        30).y+((pose_landmarks.part(29).y-pose_landmarks.part(30).y)*0.35))
    limit_y2 = round(pose_landmarks.part(
        28).y+((pose_landmarks.part(28).y-pose_landmarks.part(29).y)*0.25))
...
def hump_nose(predictor_model, file_name, pose_landmarks):
    ...
    while(b < 30):
        ...
        limit_x2 = round(
            ((y-y1)*(x2-x1)+pose_landmarks.part(a).x*(y2-y1))/(y2-y1))
        x = limit_x1
        first_color = pix[x, y]
        #print('first_color: '+str(first_color))
        average_f = (first_color[0]+first_color[1]+first_color[2])/3
...
def forehead(predictor_model, file_name, pose_landmarks):
    ...
    i = 1
    sum_changes.append(
        math.sqrt((r[i]-r[i-1])**2+(g[i]-g[i-1])**2+(b[i]-b[i-1])**2))
    sum = 0
    while(i < len(r)):
        sum_changes.append(
            math.sqrt((r[i]-r[i-1])**2+(g[i]-g[i-1])**2+(b[i]-b[i-1])**2))
    ...
    dist2 = scriptsVector.distance(start, round(
        (((y2-y1)*(start-x1))+yn*(x2-x1))/(x2-x1)), end, round((((y2-y1)*(end-x1))+yn*(x2-x1))/(x2-x1)))
...
def eyelids(predictor_model, file_name, pose_landmarks):
    ...
    while counter < 5:
        if pose_landmarks.part(b+counter).y < ym:
            ym = round(pose_landmarks.part(
                b+counter).y+(pose_landmarks.part(a+2).y-pose_landmarks.part(b+counter).y)*0.25)
...
def hair_color(predictor_model, file_name, pose_landmarks):
    ...
    limit_y1 = round(pose_landmarks.part(27).y)
    limit_y2 = round(pose_landmarks.part(
        27).y-(pose_landmarks.part(8).y-pose_landmarks.part(27).y))
    x1 = pose_landmarks.part(27).x
    x2 = pose_landmarks.part(33).x
    y1 = pose_landmarks.part(27).y
    y2 = pose_landmarks.part(33).y
    ...
    light = (0.299*second_color[0] + 0.587 *
             second_color[1] + 0.114*second_color[2])/2.55
    dark = 100-light
    orange = (abs(255-second_color[0]) +
              abs(second_color[1]-102)+second_color[2])/6.63
